...
def build_ticket_type_prompt() -> str:
    return (
        ...
        "1. **Estou com um problema** (Algo parou de funcionar ou está com erro)\n"
        "2. **Preciso de algo novo** (Acessos, equipamentos ou novas instalações)\n\n"
...
def build_query_menu() -> str:
    return (
        ...
        "1. **Chamados em aberto**\n"
        "2. **Chamados em atendimento**\n"
        "3. **Ver histórico recente**\n"
        "4. **Buscar por número (ID)**\n"
        "5. **Voltar ao início**"
    )
...
def build_invalid_option_message() -> str:
    return (
        "Não entendi a opção. Responda apenas com o **número** "
...
def build_description_review_message(
    ...
    return (
        ...
        "1. **Sim, continuar**\n"
        "2. **Não, quero ajustar o texto**\n"
        "3. **Usar meu texto original**\n"
        "4. **Cancelar**"
    )
...
def build_complement_review_message(rewritten_text: str) -> str:
    return (
        ...
        f'"{rewritten_text}"\n\n'
        "Confirmar envio?\n\n"
        "1. **Sim, adicionar ao chamado**\n"
        "2. **Não, ajustar o texto**\n"
        "3. **Cancelar**"
    )
